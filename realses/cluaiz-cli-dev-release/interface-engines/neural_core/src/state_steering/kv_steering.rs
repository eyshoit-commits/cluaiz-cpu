...
use crate::interfaces::memory_contract::{MappedBuffer, SovereignBuffer};
...
impl SovereignMapper {
    ...
    pub fn map_vault<P: AsRef<Path>>(&self, path: P) -> Result<MappedBuffer> {
        ...
        let is_ssd = self
            ...
            .storage
            .iter()
            .any(|s| s.drive_type.to_lowercase().contains("ssd"));
...
impl KVSteering {
    ...
    pub unsafe fn inject(_engine_ptr: *mut std::ffi::c_void, buffer: &dyn SovereignBuffer) {
        ...
        println!(
            "💉 [KV-Steering] Injecting zero-copy buffer: {:?} ({} bytes)",
            ptr, len
        );
    }
}
