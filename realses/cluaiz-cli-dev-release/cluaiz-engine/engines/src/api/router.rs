...
use cluaiz_shared::{
    BackendType, CluaizContext, ModelWeightsWrapper, StructuralDNA, TemplateManager, UnifiedBackend,
};
...
impl CoreRouter {
        ...
        let (tokenizer, t_error) = if let Some(p) = path.parent() {
            ...
            if t_path.exists() {
                match tokenizers::Tokenizer::from_file(&t_path) {
                    ...
                    Err(e) => (
                        None,
                        Some(format!("Tokenizer found but failed to parse: {}", e)),
                    ),
                }
            } else {
                (
                    None,
                    Some(format!("tokenizer.json missing at {:?}", t_path)),
                )
            }
        } else {
    ...
    pub fn generate_stream(
        ...
        let intent_result = rt
            .block_on(self.foundry.process_intent(prompt))
