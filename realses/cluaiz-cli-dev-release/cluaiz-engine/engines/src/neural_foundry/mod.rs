...
impl CoreFoundry {
    ...
    pub async fn process_intent(&self, prompt: &str) -> anyhow::Result<IntentResult> {
        ...
        info!(
            "🧬 [CoreFoundry] Multi-Skill Fusion Active: {} skills detected.",
            skill_ids.len()
        );
        ...
        for (i, skill_id) in skill_ids.iter().enumerate() {
            ...
            if logic_path.exists() {
                match self
                    .wasm_runtime
                    .execute_skill_logic(&logic_path, "run", prompt)
                    .await
                {
                    Ok(resp) => result.responses.push(resp),
                    Err(e) => {
                        tracing::error!("⚠️ [CoreFoundry] Logic failed for '{}': {}", skill_id, e)
                    }
                }
            }
        }
