    ...
    pub fn log_dispatch(&self, category: &ModelCategory) {
        match category {
            ModelCategory::Chat => tracing::info!(
                ...
                self.model_chat.hardware,
                self.model_chat.priority,
                self.model_chat.kv_cache_bit,
                self.model_chat.ram_buffer
            ),
            ModelCategory::Persona => tracing::info!(
                "🧠 Persona Model: {} | Balance: {} | TurboQuant: {}",
                self.model_persona.hardware,
                self.model_persona.load_balance_ratio,
                self.model_persona.turbo_quant_mapping
            ),
            ModelCategory::Embedding => tracing::info!(
                "📊 Embedding Model: {} | AVX: {} | Buffer: {}GB",
                self.profile_embedding.hardware,
                self.profile_embedding.instruction_bypass,
                self.profile_embedding.ram_buffer
            ),
            ModelCategory::Unknown => {
                tracing::info!("❓ Unknown model category — using Auto dispatch")
            }
        }
    }
}
