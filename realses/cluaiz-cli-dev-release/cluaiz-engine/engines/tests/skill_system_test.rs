...
#[cfg(test)]
mod tests {
    use engines::neural_foundry::{security::guard::PermissionLevel, IntentResult, NeuralFoundry};
    use std::fs;
    use std::path::Path;
    ...
    fn setup_mock_skills(base_path: &str) {
        ...
        let wasm_bytes = vec![
            ...
            0x7f, 0x01, 0x7f, 0x03, 0x02, 0x01, 0x00, 0x05, 0x03, 0x01, 0x00, 0x01, 0x07, 0x11,
            0x02, 0x06, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79, 0x02, 0x00, 0x03, 0x72, 0x75, 0x6e,
    ...
    async fn test_full_neural_foundry_flow() {
        ...
        assert!(
            !intent_result.signals.is_empty(),
            "❌ Neural Signals should be generated for KV_CACHE skill"
        );
        ...
        let guard_res = foundry
            .guard
            .validate_action(&skill.manifest, PermissionLevel::ReadOnly);
        assert!(
            guard_res.is_ok(),
            "❌ Security Guard should allow ReadOnly access"
        );
        println!("✅ [Test] Security Guard validated ReadOnly access.");

        // 4. Test Security Guard (Negative Case - Unauthorized Level)
        let guard_fail_res = foundry
            .guard
            .validate_action(&skill.manifest, PermissionLevel::DangerFullAccess);
        assert!(
            guard_fail_res.is_err(),
            "❌ Security Guard should BLOCK DangerFullAccess for a ReadOnly skill"
        );
