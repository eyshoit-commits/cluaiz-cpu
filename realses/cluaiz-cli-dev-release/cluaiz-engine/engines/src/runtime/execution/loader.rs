...
use cluaiz_shared::{CluaizContext, ModelWeightsWrapper, StructuralDNA, TemplateManager};
use std::path::Path;
...
impl GGUFLoader {
    pub async fn load_model(
        path: &Path,
        hf_repo: &str,
    ) -> Result<(ModelWeightsWrapper, Tokenizer, Option<u32>)> {
        // 1. Detect Architecture via Native Prober (Zero Framework Bloat)
        let (metadata, _tensor_infos) =
            GGUFProber::probe(path).map_err(|e| anyhow!("Native Probe Failure: {}", e))?;

        let arch = metadata.get("general.architecture").ok_or_else(|| {
            anyhow!("Registry Alert: Architecture metadata missing in GGUF file.")
        })?;

        // [Cluaiz CLEAN]: Replaced tracing::info with println for editor stability
        println!(
            "🔍 Autonomous Discovery: Probed architecture '{}' via Native Prober",
            arch
        );
        ...
        let model_dir = path
            .parent()
            .ok_or_else(|| anyhow!("Invalid model path structure."))?;
        let dna_path = model_dir.join("structural_dna.json");
        let architectural_dna = if dna_path.exists() {
            StructuralDNA::load(&dna_path).map_err(|load_err| {
                anyhow!("Cluaiz Boot Failure: DNA corrupt. Detail: {}", load_err)
            })?
        } else {
        ...
        let tokenizer_path =
            Provisioner::ensure_assets(model_dir, hf_repo, None, "tokenizer.json").await?;
        let tokenizer = Tokenizer::from_file(tokenizer_path)
            .map_err(|e| anyhow!("Core Hardware Error: Failed to load tokenizer: {}", e))?;

        // 🧬 Cluaiz ACTIVATION: Dynamic Context Bootstrapping
        let Cluaiz_context = CluaizContext::boot(architectural_dna, TemplateManager::default());
