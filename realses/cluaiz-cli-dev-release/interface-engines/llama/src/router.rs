...
impl BinaryRouter {
    ...
    pub fn resolve_binary() -> PathBuf {
        ...
        let driver_slug = if profile.active_drivers.iter().any(|d| d.driver_id == "CUDA") {
            "cuda"
        } else if profile
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "METAL")
        {
            "metal"
        } else if profile.active_drivers.iter().any(|d| d.driver_id == "ROCM") {
            "rocm"
        } else if profile
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "VULKAN")
        {
            "vulkan"
        } else if profile
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "OPENCL")
        {
            "opencl"
        } else if profile
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "NNAPI")
        {
            "nnapi"
        } else {
            "cpu"
        ...
        if !bin_path.exists() {
            tracing::warn!(
                "⚠️ [Router] Hardware-Specific Binary NOT found: {:?}",
                bin_path
            );
        }
    ...
    pub fn get_compute_args(requires_gpu: bool) -> Vec<String> {
        ...
        if profile
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "CUDA" || d.driver_id == "METAL" || d.driver_id == "ROCM")
        {
            if requires_gpu {
            ...
            .active_drivers
            .iter()
            .any(|d| d.driver_id == "OPENVINO" || d.driver_id == "QNN")
        {
