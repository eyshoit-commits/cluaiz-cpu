...
impl AutoTuner {
    ...
    pub fn tune(control: &mut BoosterControl, silicon: &SiliconTruth) {
        ...
        if control.flash_attention == FeatureState::Auto {
            let has_cuda = silicon
                .accelerators
                .gpus
                .iter()
                .any(|g| g.vendor.to_lowercase().contains("nvidia"));
            control.flash_attention = if has_cuda {
                FeatureState::On
            } else {
                FeatureState::Off
            };
            println!(
                "🧪 [Manager] Auto-Tuner: Flash Attention calibrated to {:?}",
                control.flash_attention
            );
        }
        ...
        if control.turbo_quant == FeatureState::Auto {
            let vram = silicon
                ...
                .sum::<f64>();
            control.turbo_quant = if vram < 8.0 {
                FeatureState::On
            } else {
                FeatureState::Off
            };
            println!(
                "🧪 [Manager] Auto-Tuner: TurboQuant calibrated to {:?}",
                control.turbo_quant
            );
        }
    }
