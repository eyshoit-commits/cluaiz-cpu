...
impl PermissionGuard {
    ...
    pub fn validate_action(
        ...
        // 📝 Audit Log
        let entry = format!(
            "[AUDIT] Skill '{}' requested level: {:?}",
            manifest.id, requested_level
        );
        self.audit_log.lock().unwrap().push(entry);

        if (skill_level as u8) < (requested_level as u8) {
            println!(
                "🚨 [SECURITY ALERT] Unauthorized Access Attempt by skill '{}'!",
                manifest.id
            );
            return Err(anyhow!(
                "Security Violation: Permission Denied for skill '{}'",
                manifest.id
            ));
        }
        ...
        if requested_level == PermissionLevel::DangerFullAccess {
            warn!(
                "⚠️ [HUMAN-IN-THE-LOOP] Critical level requested by '{}'. Verification mandatory.",
                manifest.id
            );
        }
    ...
    pub fn validate_file_access(
        ...
        if is_write && skill_level < PermissionLevel::WorkspaceWrite {
            return Err(anyhow!(
                "Security Violation: Write attempt in Read-Only mode by '{}'.",
                manifest.id
            ));
        }
        ...
        if !path.contains("Cluaiz-workspace") && skill_level < PermissionLevel::DangerFullAccess {
            return Err(anyhow!(
                "Security Violation: Out-of-bounds access attempt by '{}'.",
                manifest.id
            ));
        }
