...
use crate::core::state::{AppState, AppMode};

pub fn render_widget(state: &AppState, area: Rect, buf: &mut Buffer) {
    if state.interaction_mode == AppMode::Normal { return; }
    ...
    let list_items: Vec<ListItem> = items.iter().enumerate().map(|(i, item)| {
        let is_selected = state.palette_state.selected() == Some(i) 
            || (state.interaction_mode == AppMode::ContextPalette && state.context_palette_state.selected() == Some(i));

        let style = if is_selected {
            Style::default().fg(Color::Cyan).bg(Color::Rgb(40, 40, 40)).add_modifier(Modifier::BOLD)
        } else {
