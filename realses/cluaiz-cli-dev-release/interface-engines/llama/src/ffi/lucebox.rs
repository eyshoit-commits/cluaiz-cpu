...
impl LuceboxBridge {
    ...
    pub fn load(lib_path: &str) -> anyhow::Result<Self> {
        unsafe {
            ...
            let fn_ddtree_verify: Symbol<LuceboxDDTreeVerifyFn> =
                lib.get(b"lucebox_ddtree_verify\0")?;
    ...
    pub fn stitch_kv_layer(
        &self,
        layer_idx: i32,
        buffer: &dyn SovereignBuffer,
    ) -> anyhow::Result<()> {
        let result = unsafe {
            (self.fn_kv_stitch)(
                self.ctx_ptr,
        ...
        if result == 0 {
            ...
            Err(anyhow::anyhow!(
                "❌ Lucebox: KV-Stitching failed for layer {}",
                layer_idx
            ))
        }
    }
