...
pub type ArcConstructor = std::sync::Arc<
    dyn Fn(
            &str,          // load_path
            CluaizContext, // system context
        ) -> anyhow::Result<ModelWeightsWrapper>
        + Send
        + Sync,
>;
...
impl GlobalFeatureRegistry {
    ...
    pub fn select_runtime(
        ...
        hardware: &crate::hardware::schema::profiles::SiliconTruth,
    ) -> BackendType {
        ...
        if has_gpu && signature.is_bitnet {
            tracing::info!(
                "🔩 [Signature] 1-bit architecture on GPU detected. Routing to Runtime B."
            );
