...
pub fn render_stream(app: &AppState, area: Rect, buf: &mut Buffer) {
    ...
    let scroll = if history_len > visible_height {
    ...
    Paragraph::new(history_lines)
        ...
        .scroll((scroll, 0))
        .render(
            Rect::new(area.x + left_margin, area.y, view_width, visible_height),
            buf,
        );

    // ── PROMPT AREA ──────────────────────────────────────────────────
    let is_loading = app.is_deleting
        || !app
            .Core_engine
            .is_loaded
            .load(std::sync::atomic::Ordering::SeqCst);
    let prompt_char = if is_loading { "…" } else { "❯" };
    let prompt_color = if is_loading {
        Color::Yellow
    } else {
        Color::Cyan
    };
    ...
    // Render Input
    Paragraph::new(prompt_line).render(
        Rect::new(
            area.x + left_margin,
    ...
    if let Ok(pulse) = app.live_pulse.pulse.read() {
        let mut gpu_spans = vec![Span::styled(
            " │ GPU: ",
        ...
        if let Some(gpu) = pulse.gpus.get(0) {
            gpu_spans.push(Span::styled(
                format!(
                    "{:>4.1}% {:>4.1}°C {:>4.1}W",
                    gpu.utilization_pct, gpu.temperature_c, gpu.power_draw_watts
                ),
        ...
        let mut spans = vec![
            ...
            Span::styled(
                format!(
                    "{:>4.1}% {:>4.1}°C",
                    pulse.cpu.utilization_pct, pulse.cpu.temperature_c
                ),
                Style::default().fg(Color::Cyan),
            ),
            Span::styled(" │ RAM: ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                format!("{:>4.1} GB", pulse.ram.used_gb),
        ...
        Paragraph::new(pulse_line).render(
            Rect::new(
                area.x + left_margin,
