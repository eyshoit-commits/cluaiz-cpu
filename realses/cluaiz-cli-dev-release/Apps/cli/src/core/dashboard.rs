        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                let (
                    ...
                    let primary_gpu_temp = pulse_lock
                        .gpus
                        .get(0)
                        .map(|g| g.temperature_c)
                        .unwrap_or(0.0);
                ...
                let _gpu_color = if (vram_pct as f32) < 50.0 {
                    style::Color::Green
                } else if (vram_pct as f32) < 80.0 {
                    style::Color::Yellow
                } else {
                    style::Color::Red
                };
                let _ram_color = if ram_pct < 50.0 {
                    style::Color::Green
                ...
                let status_color = if cpu < 50.0 && (vram_pct as f32) < 50.0 {
                    style::Color::Green
                } else if cpu < 80.0 || (vram_pct as f32) < 80.0 {
                    style::Color::Yellow
                } else {
                    style::Color::Red
                };
                ...
                if show_ref.load(Ordering::SeqCst) {
                    if let Ok((_x, mut y)) = cursor::position() {
                        ...
                        let _ = execute!(
                            ...
                            style::Print(Stylize::bold(
                                format!(" {} │ ", neural_label).with(neural_color)
                            )),
                            ...
                            style::Print(Stylize::dim(format!(
                                "{:.0}°C ({:.0}%)",
                                gpu_temp, vram_pct
                            ))),
                            style::Print(Stylize::dim(format!(
        ...
        let ans = if let Some(target) = auto_boot_target {
            ...
            loop {
                ...
                if master_ans.contains("Engine Modes") {
                    ...
                    loop {
                        ...
                        let mut options = vec![
                            ...
                            format!(
                                "Speculative Decoding (Current: {:?})",
                                booster.speculative_decoding
                            ),
                        ...
                        match key_part.as_str() {
                            ...
                            "DFlash (FlashKDA)" => {
                                booster.dflash = match val_ans.as_str() {
                                    "On" => {
                                        cluaiz_shared::hardware::schema::booster::SmartState::Static(
                                            "On".to_string(),
                                        )
                                    }
                                    "Off" => {
                ...
                let downloaded: Vec<_> =
                    state.sorted_models.iter().filter(|m| m.is_cached).collect();
                ...
                let options: Vec<String> =
                    downloaded.iter().map(|m| m.manifest.name.clone()).collect();
