...
pub async fn execute(model_id: &str) -> Result<ModelManifest> {
    ...
    if manifest.is_none() {
        println!("  {} Model not found locally. Fetching registry...", "🌐".yellow());
    ...
    println!("     Architecture: {} | Params: {}", manifest.architecture.dimmed(), manifest.parameters.dimmed());
    println!("     Context: {} | VRAM Req: {} GB\n", manifest.context_window.dimmed(), manifest.ram_required_gb.to_string().dimmed());
    ...
    manager.pull_model(model_id).await
        .map_err(|e| color_eyre::eyre::eyre!(e))?;

    println!("  {} Model '{}' verified and ready. Launching dashboard...\n", "✅".green(), model_id.bold());
