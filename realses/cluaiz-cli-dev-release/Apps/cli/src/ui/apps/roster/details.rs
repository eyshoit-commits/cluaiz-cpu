    ...
    let specs_lines: Vec<Line> = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default()
                    .fg(match recommendation.status.as_str() {
                        "Optimal" => Color::Green,
...
fn render_action_bar(
    ...
    if app.show_download_confirm {
        ...
        Widget::render(
            Paragraph::new("INITIATE DOWNLOAD?").style(
                Style::default()
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::BOLD),
            ),
        ...
        Widget::render(
            Paragraph::new("[   NO   ]").centered().style(no_style),
            btn_row[3],
            buf,
        );
        Widget::render(
            Paragraph::new("[  YES   ]").centered().style(yes_style),
            btn_row[5],
            buf,
        );
        return;
    ...
    if let Some(dl_id) = &app.downloading_id {
        if dl_id == &manifest.id {
            ...
            let bar_width = area
                .width
                .saturating_sub(time_str.len() as u16 + 40)
                .max(10);
            ...
            let bar_line = Line::from(vec![
                ...
                Span::styled("] ", Style::default().fg(Color::White)),
                Span::styled(
                    format!("{:.2} MiB/{:.2} MiB ", current_mib, total_mib),
                    Style::default().fg(Color::White),
                ),
                Span::styled(
                    format!("{:.2} MiB/s", speed_mib),
    ...
    let primary_label = if recommendation.is_cached {
        "▶  RUN ENGINE"
    } else {
        "📥  DOWNLOAD GGUF"
    };
    ...
    if has_delete {
        ...
        Widget::render(
            Paragraph::new(delete_label).centered().style(delete_style),
            btn_row[3],
            buf,
        );
    }
