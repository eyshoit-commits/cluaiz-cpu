...
use crate::core::state::{AppState, OnboardingStep, OsState, AuthMode};
...
pub fn render_flow(state: &mut AppState, theme: &Theme, area: Rect, buf: &mut Buffer) {
    ...
    let footer_area = Rect::new(area.x, area.bottom().saturating_sub(footer_height), area.width, footer_height);
    let view_area = Rect::new(area.x, area.y, area.width, area.height.saturating_sub(footer_height));
    ...
    for &&step in &visible_completed {
        if current_y >= view_area.bottom() { break; }
...
fn render_active_step(state: &mut AppState, _theme: &Theme, step: OnboardingStep, area: Rect, buf: &mut Buffer) {
    match step {
...
fn render_logo_step(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    let logo_area = Rect::new(area.x + left_margin, chunks[1].y, area.width.saturating_sub(left_margin), chunks[1].height);
...
fn render_welcome_step(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    Paragraph::new(wrapped)
        ...
        .render(Rect::new(area.x + left_margin + 2, chunks[3].y, area.width.saturating_sub(left_margin + 2), 10), buf);
    ...
    if visible_chars >= ABOUT_TEXT.len().saturating_sub(10) {
        Paragraph::new("[ PRESS ENTER TO CONTINUE ]")
            .style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))
            .render(Rect::new(area.x + left_margin, chunks[5].y, area.width, 1), buf);
    }
}
    ...
    match state.auth_sub_step {
        0 => {
            ...
            Paragraph::new("  🌐  Sign in with Google  ")
                .block(Block::default().borders(Borders::ALL).border_style(
                    if google_sel { Style::default().fg(Color::Rgb(66, 133, 244)) } else { Style::default().fg(Color::DarkGray) }
                ))
                .style(if google_sel {
                    Style::default().fg(Color::Black).bg(Color::Rgb(66, 133, 244)).add_modifier(Modifier::BOLD)
                } else {
            ...
            Paragraph::new("  ✉️  Sign in with Email  ")
                .block(Block::default().borders(Borders::ALL).border_style(
                    if email_sel { Style::default().fg(Color::Cyan) } else { Style::default().fg(Color::DarkGray) }
                ))
                .style(if email_sel {
            ...
            Paragraph::new("↑↓ Navigate │ ENTER Select")
                .style(Style::default().fg(Color::DarkGray))
                .render(Rect::new(area.x + left_margin, chunks[7].y, area.width, 1), buf);
        }
        1 => {
        ...
        2 => {
            ...
            Paragraph::new(format!("  Email: {}", state.auth_email_input))
                .style(Style::default().fg(Color::DarkGray))
                .render(Rect::new(area.x + left_margin + 2, chunks[3].y, area.width, 1), buf);
...
fn render_usage_step(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    // Options
    for (i, (icon, label)) in [("👤", "Personal AI Assistant"), ("🏢", "Business & Teams")].iter().enumerate() {
        ...
        Paragraph::new(format!("  {}  {}", icon, label))
            .block(Block::default().borders(Borders::ALL).border_style(
                if is_sel { Style::default().fg(Color::Cyan) } else { Style::default().fg(Color::DarkGray) }
            ))
            .style(if is_sel {
                Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)
            } else {
...
fn render_business_profile(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    let list_area = Rect::new(area.x + lm + 2, area.y + 3, opt_w,
        area.height.saturating_sub(5));

    let field_labels = ["Business Name", "Industry", "Sub-Category", "Business Model", "Target Audience", "Primary Goal"];
    let current_label = field_labels.get(field_idx).unwrap_or(&"Complete");

    Paragraph::new(format!("🏢 Business Setup — {} ({}/{})", current_label, field_idx + 1, field_labels.len()))
        .style(Style::default().fg(Color::White).add_modifier(Modifier::BOLD))
        .render(Rect::new(area.x + lm, area.y + 1, area.width, 1), buf);
...
fn render_option_list(options: Vec<String>, selected: usize, x: u16, width: u16, area: Rect, buf: &mut Buffer) {
...
fn render_hardware_step(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    let specs: Vec<Line> = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{} ({} cores)", cpu_brand, cpu_cores), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{:.1} GB", state.ram_gb), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default()
                    .fg(if state.onboarding_status.contains("✓") { Color::Green } else { Color::Yellow })
...
fn render_complete_step(state: &mut AppState, area: Rect, buf: &mut Buffer) {
    ...
    Paragraph::new(summary)
        ...
        .render(Rect::new(area.x + lm, area.y + 3, area.width.saturating_sub(lm), 5), buf);

    let cycle = (state.frame_counter as f32 / 15.0).sin() * 0.5 + 0.5;
    let alpha = (cycle * 255.0) as u8;
    Paragraph::new("[ PRESS ENTER TO LAUNCH DASHBOARD ]")
        .style(Style::default().fg(Color::Rgb(alpha, 255, alpha)).add_modifier(Modifier::BOLD))
...
fn render_keybar(state: &AppState, step: OnboardingStep, area: Rect, buf: &mut Buffer) {
    ...
    let hint = match step {
        OnboardingStep::UsageChoice | OnboardingStep::ModelSelection | OnboardingStep::HardwareAudit => "↑↓ Navigate │ ENTER Select",
        OnboardingStep::Auth => if state.auth_sub_step == 0 { "↑↓ Choose │ ENTER Select" } else { "Type input │ ENTER Submit" },
        OnboardingStep::ProfileInfo => if state.profile_field_index == 0 && state.user_profile.account_type == AccountType::Personal { "Type name │ ENTER Submit" } else { "↑↓ Navigate │ ENTER Select" },
    ...
    let spans = vec![
        ...
        Span::styled(" ESC ", Style::default().bg(Color::DarkGray).fg(Color::Black)),
        Span::styled(" Back ", Style::default().fg(Color::DarkGray)),
    ];

    Line::from(spans).render(Rect::new(area.x + 2, area.y + 1, area.width.saturating_sub(4), 1), buf);
}
