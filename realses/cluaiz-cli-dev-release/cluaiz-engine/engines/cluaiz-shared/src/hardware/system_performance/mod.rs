...
impl SystemPerformanceLive {
    pub fn new() -> Self {
        ...
        for gpu in &control.silicon_truth.accelerators.gpus {
            if gpu.vendor.contains("NVIDIA") {
                if let Ok(d) = drivers::nvidia::NvidiaDriver::init() {
                    gpu_drivers.push(Box::new(d));
                }
            } else if gpu.vendor.contains("AMD") {
                if let Ok(d) = drivers::amd_rocm::AmdRocmDriver::init() {
                    gpu_drivers.push(Box::new(d));
                }
            } else if gpu.vendor.contains("Apple") {
                if let Ok(d) = drivers::apple_metal::AppleMetalDriver::init() {
                    gpu_drivers.push(Box::new(d));
                }
            } else if gpu.vendor.contains("Intel") {
                if let Ok(d) = drivers::openvino::OpenVinoDriver::init() {
                    gpu_drivers.push(Box::new(d));
                }
            }
        }
        ...
        for npu in &control.silicon_truth.accelerators.npus {
            if npu.model.contains("Qualcomm") || npu.model.contains("QNN") {
                if let Ok(d) = drivers::qnn::QnnDriver::init() {
                    gpu_drivers.push(Box::new(d));
                }
            }
        }
        ...
        for _tpu in &control.silicon_truth.accelerators.tpus {
            if let Ok(d) = drivers::tpu::TpuDriver::init() {
                gpu_drivers.push(Box::new(d));
            }
        }

        let cpu_driver =
            Box::new(drivers::cpu_generic::CpuGenericDriver::init()) as Box<dyn HardwareDriver>;
    ...
    pub fn tick(&mut self) {
        ...
        if let Ok(temp) = self.cpu_driver.temperature_c() {
            pulse.cpu.temperature_c = temp;
        }
        if let Ok(util) = self.cpu_driver.utilization_pct() {
            pulse.cpu.utilization_pct = util;
        }
        if let Ok(clock) = self.cpu_driver.clock_mhz() {
            pulse.cpu.clock_ghz = clock as f32 / 1000.0;
        }

        pulse.per_core_usage = sys.cpus().iter().map(|c| c.cpu_usage() as u32).collect();

        // 2. RAM Metrics
        if let Ok(used) = self.cpu_driver.vram_used_mb() {
            pulse.ram.used_gb = used as f64 / 1024.0;
        }
        if let Ok(total) = self.cpu_driver.vram_total_mb() {
            if total > 100 {
                pulse.ram.utilization_pct =
                    (pulse.ram.used_gb / (total as f64 / 1024.0)) as f32 * 100.0;
            }
        }
        ...
        if total_disk_usage > 0 {
            ...
            pulse.storage_throughput_mbps = networks
                .iter()
        ...
        for drv in &self.gpu_drivers {
            ...
            if let Ok(temp) = drv.temperature_c() {
                gpu_p.temperature_c = temp;
            }
            if let Ok(util) = drv.utilization_pct() {
                gpu_p.utilization_pct = util;
            }
            if let Ok(pwr) = drv.power_draw_watts() {
                gpu_p.power_draw_watts = pwr;
            }
    ...
    pub fn apply_booster_profile(
        &self,
        _control: &super::schema::booster::BoosterControl,
    ) -> Result<()> {
        tracing::info!(
            "🚀 [Kernel] Applying Autonomous Silicon Booster Profile: Maximum Performance"
        );
