                ...
                if choice.contains("INITIATE") {
                    ...
                    loop {
                        ...
                        if event::poll(Duration::from_millis(50))? {
                            if let Event::Key(key) = event::read()? {
                                if key.code == KeyCode::Esc || key.code == KeyCode::Char('q') {
                                    println!();
                                    let stop =
                                        Confirm::new("⚠️  Abort download and purge partial files?")
                                            .with_default(false)
                                            .with_render_config(config)
                                            .prompt();
                        ...
                        match local_rx.try_recv() {
                            Ok(event) => {
                                if let DownloadEvent::Progress(_prog, current, total, speed, _eta) =
                                    event
                                {
                                    ...
                                    let filled = if total > 0 {
                                        ((current as f64 / total as f64) * bar_width as f64)
                                            as usize
                                    } else {
                                        0
                                    };
                    ...
                    if !aborted {
                        println!(
                            "\n\n  {} {}",
                            "✔".green(),
                            "Core matrix materialized locally.".white().bold()
                        );
                        ...
                        println!(
                            "\n\n  {} {}",
                            "❌".red(),
                            "Download aborted by user.".red().bold()
                        );
                    ...
                    match confirm {
                        Ok(true) => {
                            let _ = engines::ModelDownloader::purge_model(
                            ...
                            println!(
                                "\n  {} {}",
                                "🗑️".red(),
                                "Model purged from local storage.".bold()
                            );
                            std::thread::sleep(std::time::Duration::from_millis(1200));
                            for _ in 0..3 {
                                print!("\x1B[1A\x1B[2K\r");
