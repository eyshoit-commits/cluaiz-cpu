...
impl CluaizInference for RuntimeB {
    ...
    fn inject_signals(
        ...
        println!(
            "💉 [Llama-Engine] Multi-Signal Injection Active: {} skills detected.",
            signals.len()
        );
            ...
            for (i, signal) in signals.iter().enumerate() {
                ...
                if current_offset + token_count > max_ctx {
                    ...
                    return Err(anyhow::anyhow!(
                        "CluaizSignal: Context Overflow at Skill {}",
                        i
                    ));
                }

                println!(
                    "🧵 [Llama-Engine] Stitching Skill {} ({} tokens) at offset {}.",
                ...
                for layer_idx in 0..max_layers as i32 {
                    ...
                    if let Err(e) = lucebox.stitch_kv_layer(layer_idx, &*signal.raw_data) {
                        tracing::error!(
                            "❌ [Llama-Engine] Stitching failed at Skill {}, Layer {}: {}",
                            i,
                            layer_idx,
                            e
                        );
