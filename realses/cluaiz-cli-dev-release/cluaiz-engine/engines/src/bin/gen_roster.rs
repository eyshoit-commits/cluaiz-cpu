...
fn main() {
    ...
    for line in content.lines() {
        ...
        if parts.len() >= 7 {
            ...
            let is_cloud =
                name.to_lowercase().contains("grok") || bit_depth.to_lowercase().contains("api");
            let repo = if is_cloud {
                "api:openai/grok".to_string()
            } else {
                generate_repo(name)
            };

            let arch = if name.contains('-') {
    ...
    let out_path = Path::new(
        r"c:\Users\Aryan\my\Cluaiz-workspace\Cluaiz-OS\Cluaiz-ai-CURE\engines\src\default_roster.json",
    );
