...
impl DriverProvisioner {
    ...
    pub async fn provision_for_hardware(driver_type: &str) -> Result<()> {
        ...
        let backend_id = Self::get_backend_id_for_platform(driver_type).ok_or_else(|| {
            anyhow!(
                "Driver type '{}' is not supported on this platform.",
                driver_type
            )
        })?;
        ...
        let manifest: serde_json::Value = if let Ok(resp) =
            client.get(Self::MANIFEST_URL).send().await
        {
            if resp.status().is_success() {
                ...
                tracing::warn!(
                    "⚠️ [PROVISIONER] Cloud Foundry returned {}, loading embedded registry...",
                    resp.status()
                );
                serde_json::from_str(&manifest_str).unwrap()
            }
            ...
            tracing::warn!(
                "⚠️ [PROVISIONER] Failed to connect to Cloud Foundry, loading embedded registry..."
            );
            serde_json::from_str(&manifest_str).unwrap()
        };
        ...
        let dest_filename = name_template.replace("{VERSION}", "dev-release");

        let dest_path = driver_dir.join(&dest_filename);

        println!(
            "📥 [PROVISIONER] Downloading {} driver from Cloud Foundry...",
            driver_type
        );
        ...
        if !response.status().is_success() {
            return Err(anyhow!(
                "Failed to download driver binary: {} from {}",
                response.status(),
                download_url
            ));
        }
        ...
        if !dest_path.exists() || fs::metadata(&dest_path)?.len() == 0 {
            return Err(anyhow!(
                "Driver provisioning failed: Loaded file is empty or missing."
            ));
        }
        ...
        println!(
            "✅ [PROVISIONER] {} Hardware Driver successfully deployed to Bare-Metal.",
            driver_type
        );
    ...
    pub fn get_driver_path() -> PathBuf {
        HardwareGovernor::resolve_hub_path()
