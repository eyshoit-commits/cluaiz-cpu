...
pub mod details;
pub mod table;
use crate::ui::apps::registry::table::{ColumnWidths, RegistryTable};
                    ...
                    if let Some(rec) = state.sorted_models.get_mut(idx) {
                        ...
                        if score == 0 {
                            println!(
                                "\n  {} {}",
                                "⚪".white(),
                                "Action Blocked: Model incompatible with hardware.".bold()
                            );
                        ...
                        state.sorted_models.sort_by(|a, b| {
                            let (_, score_a, _) =
                                RegistryTable::calculate_health(a, &state.hardware);
                            let (_, score_b, _) =
                                RegistryTable::calculate_health(b, &state.hardware);
                            score_b.cmp(&score_a)
                        });

                        match action {
                            Some(act) if act == "DELETE" => {
                                history_segment = Some(format!(
                                    "{} ❯ {} {}",
                                    name.dimmed(),
                                    "🗑️".dimmed(),
                                    "Deleted".dimmed()
